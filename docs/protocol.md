...
- `_ping` — health check used by `host_status` and the `host_launch` / `host_changelevel` readiness waits. Bypasses the `mcp_enable` gate. Returns `{ ok = true, enabled = <mcp_enable bool>, realm = "server"|"client", map = <current map>, maxplayers = <int>, singleplayer = <bool>, generation = <int>, bootstrap_pending = <bool>, bootstrap_error = <string|absent>, bootstrap_map_missing = <string|absent>, capabilities = { <cap-id> = <granted bool>, ... }, has_focus = <bool, client realm only> }`. `maxplayers`/`singleplayer` let the host report listen-server vs singleplayer — and, because maxplayers is fixed at launch, signal that switching modes needs a relaunch. `bootstrap_pending` is true while a `host_launch` intent or a `_changelevel` transition is queued or mid-flight. `bootstrap_error` is set (and `bootstrap_pending` cleared) when a transition fails terminally — e.g. a `_changelevel` that never starts — so the host fails fast instead of waiting out its timeout; absent when unset. `bootstrap_map_missing` is the **soft** counterpart on the `host_launch` bootstrap path: the requested map was neither on disk nor provided by a mounted workshop addon, so it doesn't exist — the launch still succeeds on the bootstrap map (`gm_construct`), readiness is **not** failed, and this carries the missing map's name back so the host can report it; absent when unset. `has_focus` (`system.HasFocus()`) is present **only on the client realm** (the server has no window); the host pairs it with its own `GetForegroundWindow` check to detect and fix GMod's stuck mouse-grab after a background launch (see AGENTS.md "Process tracking"). `capabilities` carries the **live** grant state of every registered capability — each `mcp_allow_*` convar read fresh at ping time — so `host_status` reports current grants rather than the manifest snapshot (which only refreshes on reload); `generation` advances on `MCP:Reload`. The readiness waits poll `_ping` on **both** realms and only return ready when both are. Older addon builds may omit the optional fields; the host decodes them as null.
