...
4. **Have the assistant launch GMod** via the `host_launch` tool, or start GMod yourself. The bridge polls regardless of consent, but tool dispatch requires opting in via convars in the GMod developer console:
   ...
   mcp_allow_unsafe 1
   ```
   These are `FCVAR_ARCHIVE`, so once set they persist across game restarts — as long as GMod is shut down cleanly. It writes these to `cfg/server.vdf` only on a clean window-close, never on a kill, so `host_close` does a clean shutdown by default (`force: true` skips it and loses grants set that session). `host_launch` waits until both the server and client realms are ready before returning, and on Windows — if a background launch leaves GMod's window holding your mouse (a GMod/SDL startup-focus bug) — it detects and releases the cursor with a brief focus flicker (opt out with `fix_focus: false`).
