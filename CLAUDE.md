...
`mcp_reload` is still available for forced rebuilds (e.g. when a tool file has been *deleted* — autorefresh has nothing to fire on for removals, and for a brand-*new* server file, which `mcp_reload` picks up because `MCP:Reload` re-runs `LoadFolder("functions")`).

**Adding a new tool (or renaming/reshaping one) — restart the Claude Code session before it's callable from Claude.** Claude Code's support for `notifications/tools/list_changed` is *partial and unreliable* (added in 2.1.0; gaps tracked in anthropics/claude-code#4118 and #31893): a newly-registered tool is only meant to appear on a *later* prompt turn, never within the current turn, and the re-fetch is known-flaky (a startup/registration race). In this project it did **not** refresh across turns in practice — a `mcp_reload` registered the tool game-side and `host_status` showed the new count, but Claude couldn't call it until the session was restarted. So treat a **session restart as the reliable way** to pick up a new or renamed tool. *Editing the body of an already-registered tool* needs no restart — autorefresh updates the handler and the name/schema Claude already knows still dispatches to it. (Distinct from the genuinely-unsupported `notifications/message` server→model channel behind the README's "Console & error capture" workaround.)
