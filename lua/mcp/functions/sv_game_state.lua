...
MCP:AddFunction({
    id = "game_state",
    description = "Structured snapshot of server-wide game state in one read -- current map, gamemode, hostname, singleplayer/dedicated flags, max player slots, player/bot/human counts, a lean roster of every player (name/userid/entindex/is_bot/is_host/team -- drill into one with player_state or entity_state), a `tuning` block with the live values of game_set's knobs (gravity, timescale, phys_timescale, fakelag), and `cheats_enabled` (sv_cheats) -- which gates whether game_set's timescale/fakelag will take. `cheats_enabled` is read-only here AND everywhere on a running game: sv_cheats is blocklisted from every Lua/console path, so it can't be flipped mid-session via the bridge -- toggle it only in the real in-game console. Note host_launch games come up with cheats already ON: it runs a LAN local host (+sv_lan 1), where sv_cheats defaults to 1. The read-half paired with game_set (which writes the tuning knobs). Read-only.",
    schema = {
        type = "object",
        properties = {},
    },
