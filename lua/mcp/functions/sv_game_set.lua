        ...
        MCP:Settle({
            ...
            for name, item in pairs(requested) do
                ...
                if item.def.cheat and not res.took then
                    res.note = "cheat-protected; needs sv_cheats 1, which is blocklisted from Lua -- enable it in the in-game console (host_launch games default to cheats on; sv_cheats left unchanged)"
                end
                knobs[name] = res
